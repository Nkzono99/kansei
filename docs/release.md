...
- `kansei init` の標準動線を `uvx --from kansei kansei <command>` に変更し、
  project-local な Kansei `.venv` は `--bootstrap` の opt-in にしました。
- `.kansei/manifest.toml` に uvx runner metadata と最後に適用した
  `harness.kansei_version` を記録するようにしました。
- PyPI latest、実行中 runtime、instance に最後に適用された harness version を比較し、
  必要な場合に `update-harness --plan` と `kansei-control-plane` skill へ誘導する
  update notice を追加しました。
- `kansei update-harness --plan`, `--apply-chain`, `--target`, `--allow-major` を追加し、
  minor checkpoint ごとに exact version の `uvx --from kansei==<version>` を呼ぶ
  versioned upgrade chain を実装しました。
- generated `providers.toml`, `.codex/config.toml`, instance README, instance skill guidance を
  uvx 標準動線に更新しました。
- repo-local HarnessOps bridge skills を最新 HarnessOps に合わせて更新しました。

互換性:

- 通常の近い更新は従来どおり `kansei update-harness` / `--apply` で preview/apply できます。
- 大きく離れた instance は `kansei update-harness --plan` で chain を確認し、
  `--apply-chain` で checkpoint ごとの更新に委譲します。
- major version を跨ぐ chain 実行には `--allow-major` が必要です。
